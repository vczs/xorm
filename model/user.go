...
type Comm struct {
	Create time.Time `xorm:"created 'ct'"` // 在Insert时自动赋值为当前时间 指定列名
	Update time.Time `xorm:"updated 'ut'"` // 在Insert或Update时自动赋值为当前时间 指定列名
	Delete time.Time `xorm:"deleted 'dt'"` // 在Delete时设置为当前时间并且软删除 指定列名
}
